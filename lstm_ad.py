...
class LSTMAD(Detector):
    """ LSTM-AD implementation using PyTorch.
    The interface of the class is sklearn-like.
    """

    def __init__(self, name='LSTM-AD',len_in=1, len_out=10, num_epochs=100, lr=1e-3, batch_size=1,
                 seed: int=None, gpu: int=None, details=True):
        super(LSTMAD, self).__init__(__name__, name, seed, details=details)
        if gpu is not None:
    ...
    def predict(self, ts, aggregate=None):
        X = pd.DataFrame(ts)
        X.interpolate(inplace=True)
        X.bfill(inplace=True)
        self.model.eval()
        ...
        errors = errors.mean(axis=-1).squeeze(0)
        scores = np.pad(errors, ((self.len_out, 0), (0, 0)), mode='minimum')
        return scores
    ...
    def _input_and_target_data(self, X: pd.DataFrame):
        ...
        input_data = torch.from_numpy(X[:, :-self.len_out, :]).to(self.device)
        target_data = []
        for l in range(self.len_out - 1):
            target_data += [X[:, 1 + l:-self.len_out + 1 + l, :]]
        target_data += [X[:, self.len_out:, :]]
        target_data = torch.from_numpy(np.stack(target_data, axis=3)).to(self.device)

        return input_data, target_data

    def _input_and_target_data_eval(self, X: pd.DataFrame):
        X = np.expand_dims(X, axis=0)
        input_data = torch.from_numpy(X).to(self.device)
        target_data = torch.from_numpy(X[:, self.len_in + self.len_out - 1:, :]).to(self.device)
        return input_data, target_data
    ...
    def _build_model(self, d, batch_size):
        self.model = LSTMSequence(d, batch_size, len_in=self.len_in, len_out=self.len_out).to(self.device)
        self.model.double()
